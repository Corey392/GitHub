...
<h2 class="center"> ${title} Status for ${user.firstName} ${user.lastName}</h2>
...
<form name="listClaimRecords" action="<%= RPLServlet.STUDENT_LIST_CLAIM_RECORDS.absoluteAddress %>?view=View Records" method="post">
    <div class="tablecontrols">
        Claim Type:&nbsp;<select name="claimType" id="cmbClaimType">
                             <option value="" >All</option>
                             <option value="0" >RPL</option>
                             <option value="1" >Previous Studies</option>  
                         </select>
        Status:&nbsp;<select name="workResult" id="cmbWorkResult">
                             <option value="" >All</option>
        ...
        &nbsp;<a class="button" href="#" onclick="listClaimRecords.submit()"><span>View Records</span></a>
    </div>
        ...
        <c:choose> 
            <c:when test="${claimRecords != null && claimRecords.size() != 0}">
                ...
                <tr>
                    ...
                    <td><span class="studentID">${cr.studentID}</span><br/>(${cr.studentName})</td>
                    <td>${cr.courseName}&nbsp;</td>
                    <td>${cr.campusName}</td>
                    <td>${cr.workTime.replace(" ", "&nbsp;")}</td>
                    <td>${cr.workerID}&nbsp;(${cr.workerName})</td>
            ...
            <c:otherwise>
                <tr>
                    <td colspan="7">You have no claim records.</td>
                </tr>
            </c:otherwise>
        </c:choose>
    </table>
